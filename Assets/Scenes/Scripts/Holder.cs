...
public class Holder : MonoBehaviour
{
    ...
    public bool IsCompleted() => (IsFull && CheckIfAllLiquidAreSame(_liquids));

    private bool CheckIfAllLiquidAreSame(List<Liquid> arrLiquids){
        HashSet<Liquid> s = new HashSet<Liquid>();
        for (int i = 0; i < arrLiquids.Count; i++)
            s.Add(arrLiquids[i]);
        return (s.Count == 1);
    }
    ...
    public IEnumerator MoveAndTransferLiquid(Holder holder, Action onLiquidTransferComplete = null){
        ...
        if (holder.IsFull || !_liquids.Any() ||
            holder.Liquids.Any() && holder.Liquids.Last().GroupId != Liquids.Last().GroupId){
            yield break;
        }
        ...
        LevelManager.Instance.AddLatestMove(this, holder, TopLiquid, transferValue);
    ...
    public void RemoveLiquid(Liquid liquid, float value = 0){
        _liquids.Last().Value -= value;
        // liquid.Value -= value;
    }
    ...
    public void MoveTo(Vector2 point, float speed = 1, Action onFinished = null){
        ...
        _moveCoroutine =
            StartCoroutine(SimpleCoroutine.CoroutineEnumerator(MoveToEnumerator(point, speed), onFinished));
    }

    private IEnumerator MoveToEnumerator(Vector2 toPoint, float speed = 1){
        var startPoint = transform.position;
        yield return SimpleCoroutine.MoveTowardsEnumerator(
            onCallOnFrame: n => { transform.position = Vector3.Lerp(startPoint, toPoint, n); }, speed: speed);
    }
}